...
def simple_model(X, y):
    ...
    W1 = tf.get_variable('W1', shape = [5408, 10])  # new_size=(32-7-1)/2+1 D=new_size*new_size*32
    b1 = tf.get_variable('b1', shape = [10])

    # define our graph (e.g. two_layer_convnet)
    a1 = tf.nn.conv2d(X, Wconv1, strides = [1, 2, 2, 1],
                      padding = 'VALID') + bconv1  # 'VALID' means no padding and drop out remnant lines.
    h1 = tf.nn.relu(a1)
...
# define our loss
total_loss = tf.losses.hinge_loss(tf.one_hot(y, 10), logits = y_out)  # SVM loss
...
def complex_model(X, y, is_training):
    # define weights    ( init_three_layer_convnet)  conv-affine-affine
    ...
    # define our graph  (three-layer-convnet)
    a1 = tf.nn.conv2d(X, Wconv1, strides = [1, 1, 1, 1], padding = 'VALID') + bconv1  # 26X26X32
    h1 = tf.nn.relu(a1)
    z1 = tf.layers.batch_normalization(h1)
    max1 = tf.layers.max_pooling2d(z1, (2, 2), 2)  # 13X13X32
    max1_flat = tf.reshape(max1, [-1, 5408])
    a2 = tf.matmul(max1_flat, W1) + b1
    h2 = tf.nn.relu(a2)
