...
def rnn_backward(dh, cache):
    ...
    ##############################################################################
    x,Wx,Wh,prev_h,next_h=cache[-1]     # get the last cache
    _,D=x.shape
    N,T,H=dh.shape
    dx=np.zeros((N,T,D))
    dh0=np.zeros((N,H))
    ...
    for i in range(T-1,-1,-1):          # from T-1 to 0 minus one every step.
...
def word_embedding_forward(x, W):
    ...
    ##############################################################################
    N,T=x.shape
    V,D=W.shape             # V represents idx of words, D is the word representation in number.
    out=np.zeros((N,T,D))
    for n in range(N):
        for t in range(T):
            out[n,t,:]=W[x[n,t]]
